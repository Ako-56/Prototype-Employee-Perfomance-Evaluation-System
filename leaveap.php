...
<?php
	if(isset($_POST['approve'])){
		...
		foreach($leav AS $all){
			$Qvq = mysqli_query($conn,"UPDATE empleave SET SuperV='Approved' WHERE Id='$all'");
			if($Qvq){
		...
		<div>
			<form method="post">
				<table>
					...
					<?php 
					$Qva = mysqli_query($conn,"SELECT Id,SuperV,Status FROM empleave WHERE SuperV='Pending'");
					...
					while($rows=mysqli_fetch_assoc($Qva)){
						...
						?>
						<tr><td colspan=4 align="center"> No Leaves to approve</td>
						<?php
					}
					?>
				</table>
